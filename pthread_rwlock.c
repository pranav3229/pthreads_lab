...
void* assign_random(void* param) {
    ...
    printf("Thread id %d\tRandom Number: %d\n", threadId, randNum);
    printf("Global min = %d\n",global_min);
    // Acquire read lock to read global_min
    mylib_rwlock_rlock(&min_lock);
    if (randNum < global_min) {
        // Release read lock and acquire write lock to update global_min
        mylib_rwlock_unlock(&min_lock);
        mylib_rwlock_wlock(&min_lock);
        if (randNum < global_min) {
            global_min = randNum;
        }
        mylib_rwlock_unlock(&min_lock);
