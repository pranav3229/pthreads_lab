...
mylib_barrier_t myBarrier;
int sum = 0;
int numbers[NTHREAD] = {1, 2, 3, 4, 5}; // Example numbers
...
int main(int argc, char const* argv[]) {
    ...
    int i = 0;
    for (i = 0; i < NTHREAD; i++) {
        pthread_create(&threadArr[i], NULL, &compute_sum, &threadIdArr[i]);
    }
...
void* compute_sum(void* param) {
    int threadId = *(int*)param;
    int portion = numbers[threadId];
    printf("Thread %d computes portion %d.\n", threadId, portion);

    // Add portion to the sum
    sum += portion;
